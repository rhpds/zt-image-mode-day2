...
Let's do something radical to explore changing the starting bootc image. 
We have a web server image that was created using RHEL 9 that needs an update.
While we could just use a later minor version, let's see what happens if we use the RHEL 10 bootc base instead.
...
Let’s connect to the virtual machine via SSH. 

==== _Switch to the `Image mode host` tab._
...
[NOTE]
====
If something went wrong with the push step, you may get an error about an unknown manifest. If so, return to the `Builder` tab and double check that the push to the lab registry was successful.
====
...
Once a host is running, changing the image in use is very straightforward, and can be useful for a number of reasons beyond radical changes like major versions. We'll stick with RHEL 10 from here, but let's explore a little more of what `bootc switch` can do for us in the next few exercises.
