...
=== _Return to the `Builder` tab to explore what happened_
...
image mode basics lab,window=_blank], we discussed how `+/etc+` is treated in image
...
<2> The commands to be executed by the `RUN` directive

Outside of a lab, you'd likely put the `echo` command that 
creates our `index.html` file inside the `heredoc` instead of including it as
...
update the httpd config file to change the default document root to our
new directory. 
...
=== _To apply this update, return to the `Image mode host` tab._
